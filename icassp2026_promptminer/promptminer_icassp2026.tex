...
the complete tested configuration attains the highest mask AP and AP$_{75}$
among the ablated variants.
\end{abstract}
\begin{keywords}
...
mask prompts~\cite{sam}. Unlike SAM's single-scale ViT encoder, SAM2 couples
its native prompt interface with Hiera multi-level features~\cite{sam2}, a
natural fit for proposal-pyramid construction and prompt-conditioned mask
decoding. A
...
features. SAMRefiner instead addresses universal mask refinement: given an
initial coarse mask, it excavates distance-guided points, elastic boxes, and
Gaussian-style mask prompts for a subsequent SAM refinement step~\cite{samrefiner}.
PointRend likewise prioritizes uncertain locations for point-wise prediction,
but does so through iterative subdivision~\cite{pointrend}. These approaches
do not specify how proposal-local shape information can be constructed and
used as aligned SAM2 prompts within a two-stage instance-segmentation model for
crowded overhead imagery.
...
and dense prompts for adjacent buildings. Figure~\ref{fig:arch} presents the
complete architecture and the data flow from proposal discovery to mask decoding.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{总体架构图.pdf}
\caption{Complete architecture of PromptMiner-SAM2. CSPM is the coarse-shape prompt mining module that converts each proposal-local coarse shape into point, box, and dense-mask prompts; UDPR is the uncertainty-guided decoder-tail refinement module that selectively refines uncertain decoder logits.}
\label{fig:arch}
\end{figure*}
...
Hiera-Base+ weights: the base Hiera parameters remain frozen and its LoRA
adapters and the mask decoder are updated. 
We use AdamW
with an initial learning rate of $5\times10^{-4}$ and cosine annealing.
Training uses cross-entropy, Smooth L1, binary cross-entropy, and Dice losses.

We select $K=64$ by screening $K\in\{32,64,128\}$ under a common protocol;
it achieves the highest overall bbox AP and mask AP in
Table~\ref{tab:k-sensitivity} and is used in the main experiments.

\begin{table}[t]
\caption{Sensitivity of $K$ on NWPU VHR-10 under a common development protocol. AP (\%).}
...
We evaluate on the WHU dataset~\cite{whu}, a single-class building benchmark with
2,943, 627, and 2,220 training, validation, and test images, and NWPU VHR-10~\cite{nwpu}, a
...
AP$_{75}$. To assess the proposed method, we compare it with
10 representative instance-segmentation methods. These
...
\midrule
\multirow{5}{*}{\textit{CNN-based}} & Mask R-CNN~\cite{maskrcnn} & \metrictriplet{70.9}{89.9}{80.9} & \metrictriplet{68.2}{90.0}{80.0} & \metrictriplet{65.4}{90.2}{78.9} & \metrictriplet{63.0}{90.1}{71.2} \\
& MS R-CNN~\cite{msrcnn} & \metrictriplet{68.8}{89.4}{80.0} & \metrictriplet{67.7}{89.6}{79.5} & \metrictriplet{67.8}{94.3}{83.8} & \metrictriplet{64.7}{93.6}{80.8} \\
...
Table~\ref{tab:comparison} summarizes the quantitative results.
PromptMiner-SAM2 obtains the highest mask AP among the
compared methods on both benchmarks. On WHU, it reaches 73.4\% mask AP, and 85.5\% mask AP$_{75}$. Its 76.4\% bbox AP
is 0.3 points below the highest bbox AP in this comparison. On NWPU VHR-10, it obtains
the highest bbox AP (72.8\%), mask AP (67.8\%), mask AP$_{50}$ (95.4\%), and
mask AP$_{75}$ (74.0\%) among the compared methods. The latter is 2.5 points
above the next-highest mask AP$_{75}$.

Figure~\ref{fig:qualitative} juxtaposes the input image, ground truth, seven
reference methods, and PromptMiner-SAM2 in a WHU building scene and a
multi-class NWPU VHR-10 scene, providing visual context for the comparison.

\subsection{Ablation Study}
Table~\ref{tab:ablation} compares prompt compositions and decoder-tail
refinement under the same protocol. From $P$ to $P{+}B$, mask
AP/AP$_{75}$ increases from 63.0/67.4\% to 65.3/71.7\%. Adding $M$ further
raises mask AP to 65.9\%, but reduces AP$_{75}$ from 71.7\% to 71.5\%.
With UDPR, the full configuration attains 67.8\% mask AP and 74.0\%
AP$_{75}$, the best mask results in this ablation.

Overall, enriching the prompt path raises mask AP from 63.0\% to 65.9\%
before refinement, while UDPR further improves both mask AP and AP$_{75}$.
The complete configuration also achieves the strongest bbox AP and bbox
AP$_{75}$ in Table~\ref{tab:ablation}, indicating that the full design
improves the final instance-segmentation results under the shared setting.
...
\section{CONCLUSION}
PromptMiner-SAM2 integrates two-stage proposals with SAM2 through CSPM and
UDPR. CSPM converts proposal-local shapes into aligned point, box, and
dense-mask prompts, reducing ambiguity between adjacent buildings. UDPR
selectively refines low-confidence decoder logits. The resulting design
