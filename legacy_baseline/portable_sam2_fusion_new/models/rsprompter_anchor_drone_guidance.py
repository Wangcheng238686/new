...
from uav import build_cvt_encoder
...
class RSPrompterAnchorDroneGuidance(RSPrompterAnchor):
            ...
            if share_sam2_backbone:
                if hasattr(self, 'backbone'):
                    if hasattr(self.backbone, 'encoder') and self.backbone.encoder is not None:
                        ...
                        logger.info("Using shared SAM2 encoder from satellite backbone")
                    else:
                        logger.warning("backbone exists but encoder is None, will create standalone SAM2")
                else:
                    logger.warning("backbone not found, will create standalone SAM2")
    ...
    def _compute_standard_loss(
        ...
        height_loss = self._build_height_loss(batch_data_samples)
    ...
    def _build_height_loss(self, batch_data_samples: SampleList):
        total_height_loss = getattr(self, "_height_loss", None)

        if (
            self.height_loss_weight > 0
            and self.guidance is not None
            and hasattr(self.guidance, "compute_supervised_height_loss")
        ):
            building_mask = self._build_building_mask_from_samples(
                batch_data_samples,
                device=self._height_map.device if getattr(self, "_height_map", None) is not None else None,
            )
            supervised_height_loss, supervised_height_dict = self.guidance.compute_supervised_height_loss(building_mask)
            if supervised_height_loss is not None:
                total_height_loss = (
                    supervised_height_loss
                    if total_height_loss is None
                    else total_height_loss + supervised_height_loss
                )
                current_height_dict = getattr(self, "_height_loss_dict", None) or {}
                current_height_dict.update(supervised_height_dict)
                self._height_loss_dict = current_height_dict

        return total_height_loss

    @staticmethod
    def _build_building_mask_from_samples(batch_data_samples: SampleList, device=None):
        ...
        for data_sample in batch_data_samples:
            img_h, img_w = data_sample.metainfo.get("img_shape", data_sample.metainfo.get("batch_input_shape", (None, None)))
            if img_h is None or img_w is None:
                return None
            target_h, target_w = int(img_h), int(img_w)

            gt_instances = getattr(data_sample, "gt_instances", None)
            gt_masks = getattr(gt_instances, "masks", None) if gt_instances is not None else None

            if gt_masks is None:
                building_masks.append(torch.zeros(1, target_h, target_w, dtype=torch.float32, device=device))
                continue

            if hasattr(gt_masks, "to_tensor"):
                mask_tensor = gt_masks.to_tensor(dtype=torch.float32, device=device)
            elif hasattr(gt_masks, "masks"):
                mask_tensor = torch.as_tensor(gt_masks.masks, dtype=torch.float32, device=device)
            else:
                mask_tensor = torch.as_tensor(gt_masks, dtype=torch.float32, device=device)

            if mask_tensor.numel() == 0:
                union_mask = torch.zeros(1, target_h, target_w, dtype=torch.float32, device=device)
            else:
                if mask_tensor.dim() == 2:
                    mask_tensor = mask_tensor.unsqueeze(0)
                union_mask = mask_tensor.amax(dim=0, keepdim=True)

            building_masks.append(union_mask)

        return torch.stack(building_masks, dim=0)
