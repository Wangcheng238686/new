...
def _resolve_depth_anything_v2_dir() -> Optional[Path]:
    """Locate the vendored Depth Anything V2 package across sibling worktrees."""
    candidate_dirs = [
        Path(__file__).resolve().parents[1] / "depth_anything_v2",
        Path(__file__).resolve().parents[2] / "portable_sam2_fusion" / "depth_anything_v2",
    ]
    for candidate in candidate_dirs:
        if (candidate / "depth_anything_v2" / "dinov2.py").exists():
            return candidate
    return None


DEPTH_ANYTHING_V2_DIR = _resolve_depth_anything_v2_dir()
if DEPTH_ANYTHING_V2_DIR is not None and str(DEPTH_ANYTHING_V2_DIR) not in sys.path:
...
class LightweightDepthEncoder(nn.Module):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "Depth Anything V2 not found. Checked local and sibling vendored paths; "
                "please install it or set depth_encoder.enabled=False"
            )
