...
## 12. 暂停记录（2026-09-18 19:36，用户指令：让路其他任务，保留 resume 能力）

**hybrid（a3sgm 尾×roi_local）终止于 E10**：0 非有限；val 轨迹
0.4887→0.6106→0.6145→0.6087→0.6385→0.6515→0.6568→0.6417→**0.6719**（E10
已破 A3-fi 天花板 0.6691——第一道关提前通过）；`last_checkpoint.pth`（E10）在
位，**resume = `RESUME_OWN=<该 last_checkpoint.pth>` 重跑同一 wrapper**。

**cur150_probe（fast150 配方×当前代码×150ep×fp32）终止于 E1 后半**：
**E1 val = 0.5375**（vs fast150 原版 0.3680）——**非模块因素坐实**：同配方同
schedule 同 P2BRR，仅代码版本+机器+精度差异即 +0.17 早期加速，§11/§10 的
"快起步归因"由此闭环（代码静态审计全绿的前提下，残余解释=机器环境/数值栈）。
probe 无 last ckpt（fast150 wrapper 仅 best-only），无需 resume。

四卡全空（GPU0 外部任务同刻结束），GPU 归还用户调度。
