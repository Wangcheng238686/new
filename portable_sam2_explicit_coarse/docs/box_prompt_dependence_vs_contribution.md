...
**A3SGM（耦合 margin 尾 + 解耦裁剪，2026-09-17 完赛 + 四权重推理）：双门通过，
全矩阵新冠军**。arm `a3sgm` = A0-SG 基座 + UDPR 协同引擎（梯度入主干）+ 边界锚定
margin 损失 + 裁剪分解（vs a3sgtm-dclip 唯一模型差异 = mode `residual_v1_margin`，
指纹 ed7a422ddf36）。**三个已证明正项首次合体**（协同 +0.012 / margin 写入 +0.0091
CI 显著 / dclip +0.026），子代理审计 PASS。

| 权重 | segm/mAP | AP50 | AP75 | bbox/mAP | bAP75 | sAR100 | composite |
|---|---:|---:|---:|---:|---:|---:|---:|
| bestS | 0.6933 | 0.9540 | 0.7752 | 0.6878 | 0.8302 | 0.7306 | 0.6905 |
| bestB=bestC | 0.6828 | 0.9453 | 0.7461 | 0.7343 | 0.8416 | 0.7184 | 0.7085 |
| last(E300) | 0.6816 | 0.9465 | 0.7492 | 0.7317 | 0.8438 | 0.7174 | 0.7067 |

**预注册门判定（对 a0_sg）**：segm **+0.0333**（门 +0.005 的 6.7 倍）✅、bbox
**−0.0031**（门 ±0.005）✅——**"A0-SG 基座上有增益的 UDPR、两指标兼得"单阶段
首次实现**。全矩阵排名刷新：composite last 0.7067（前冠军 A3 0.6943，+0.0124）、
bestC 0.7085（A3 0.6994，+0.0092；两阶段热启动 0.6999，+0.0086）。**论文主行
= a3sgm，不再需要两阶段口径**。

三个机制预言全部兑现：① A5 塌陷窗口（E30–45）不降反升（E35 达 0.6915）——
dclip 拆掉死亡螺旋主轴的救援假设成立；② 协同引擎 × margin 信号放大：E35 同期
0.6915 vs a0_sg 0.5877，协同红利从 a3sg 的 +0.012 放大到 +0.033；③ bbox 收敛
在门内（0.7317，距基座最高值 −0.003）。产物：
`ablations/vhr10_p2v2_matrix300_a3sgm_pbm_sg_udprk64tm_tr1.0_va1.0/inference_*_vhr10/`。

**非尾臂标定重评测（2026-09-17，6 臂 × 4 权重，四卡并行）**：协议 = 每臂在
last 权重 train 520 图上扫 {0.40–0.65} 取 argmax（标定源），val 130 图全权重
重推（报告集，零选择泄漏）。探针 `inference/probes/mask_threshold_sweep.py`
（train/validation 双 split）。a0_sg 已预验 train-argmax = val-argmax = 0.6。

| 臂 | thr | last segm@0.4 | last segm@cal | Δ | bestS@cal | bestC@cal |
|---|---:|---:|---:|---:|---:|---:|
| p | 0.60 | 0.6247 | 0.6273 | +0.0026 | 0.6430 | 0.6748 |
| b | 0.55 | 0.6345 | 0.6435 | +0.0090 | 0.6584 | 0.6869 |
| pb | 0.60 | 0.6392 | 0.6407 | +0.0015 | 0.6694 | 0.6934 |
| a0 | 0.60 | 0.6331 | 0.6410 | +0.0079 | 0.6683 | 0.6868 |
| a0_sg | 0.60 | 0.6483 | 0.6579 | +0.0096 | 0.6751 | 0.7006 |
| r3 | 0.60 | 0.6271 | 0.6341 | +0.0070 | 0.6693（+0.0260） | 0.6740 |

标定口径判读（B 阈值说明：B 的 train-split 扫描 argmax 在 0.55——0.55:0.7708 > 0.60:0.7704，差仅 0.0004，且 val 侧两阈值近持平——纯框模式的掩膜概率分布与点模式不同，标定峰略左移；协议按每臂 train-argmax 取 0.55，非全局统一值）：① 非尾臂 last 全部为正（+0.002~+0.010），bestS/bestC 权重普遍
+0.005~+0.026（r3 的 E70 尖峰权重 +0.026 为最大）；② a0_sg bestC@cal 0.7006
——非尾臂最佳 composite，距 a3sgm 的 0.7085 差 0.008，即 UDPR 包在标定口径
下的净贡献；③ 尾臂（A3 系）自标定于 0.4 不参与（margin 锚定守卫强制一致）；
④ pb 的 train-argmax 0.60 迁移略亏（val 最优 0.55，差 −0.003）——标定迁移
代价在 0.003 量级。工件已归档 `diagnostics/nontail_calibrated_reeval_20260917/`
（含完整全字段 JSON `full_calib_metrics.json` 与逐臂 worker 日志）。

**非尾臂标定口径完整 COCO 四权重表（2026-09-17 四卡并行重跑；bbox 列沿用原
推理——阈值不影响检测器字段）**：

**p（标定阈值 0.60）**
| 权重 | segm/mAP | AP50 | AP75 | AR100 | bbox/mAP | comp |
|---|---:|---:|---:|---:|---:|---:|
| bestS(E155) | 0.6430 | 0.9410 | 0.6932 | 0.6890 | 0.7024 | 0.6727 |
| bestB(E250) | 0.6309 | 0.9226 | 0.6800 | 0.6715 | 0.7197 | 0.6753 |
| bestC(E280) | 0.6315 | 0.9256 | 0.6773 | 0.6727 | 0.7181 | 0.6748 |
| last(E300) | 0.6273 | 0.9218 | 0.6838 | 0.6692 | 0.7161 | 0.6717 |

**b（标定阈值 0.55）**
| 权重 | segm/mAP | AP50 | AP75 | AR100 | bbox/mAP | comp |
|---|---:|---:|---:|---:|---:|---:|
| bestS(E185) | 0.6584 | 0.9306 | 0.7068 | 0.6979 | 0.7089 | 0.6836 |
| bestB=bestC | 0.6525 | 0.9412 | 0.6902 | 0.6911 | 0.7214 | 0.6869 |
| last(E300) | 0.6435 | 0.9220 | 0.6859 | 0.6826 | 0.7140 | 0.6787 |

**pb（标定阈值 0.60）**
| 权重 | segm/mAP | AP50 | AP75 | AR100 | bbox/mAP | comp |
|---|---:|---:|---:|---:|---:|---:|
| bestS=bestC(E155) | 0.6694 | 0.9393 | 0.7273 | 0.7088 | 0.7174 | 0.6934 |
| bestB(E235) | 0.6464 | 0.9428 | 0.6834 | 0.6850 | 0.7276 | 0.6870 |
| last(E300) | 0.6407 | 0.9332 | 0.6796 | 0.6774 | 0.7215 | 0.6811 |

**a0（标定阈值 0.60）**
| 权重 | segm/mAP | AP50 | AP75 | AR100 | bbox/mAP | comp |
|---|---:|---:|---:|---:|---:|---:|
| bestS(E105) | 0.6683 | 0.9358 | 0.7315 | 0.7049 | 0.6963 | 0.6823 |
| bestB=bestC(E265) | 0.6451 | 0.9373 | 0.6776 | 0.6838 | 0.7284 | 0.6868 |
| last(E300) | 0.6410 | 0.9355 | 0.6693 | 0.6824 | 0.7218 | 0.6814 |

**a0_sg（标定阈值 0.60）**
| 权重 | segm/mAP | AP50 | AP75 | AR100 | bbox/mAP | comp |
|---|---:|---:|---:|---:|---:|---:|
| bestS(E155) | 0.6751 | 0.9438 | 0.7460 | 0.7112 | 0.7183 | 0.6967 |
| bestB=bestC(E225) | 0.6635 | 0.9366 | 0.7161 | 0.7041 | 0.7376 | 0.7006 |
| last(E300) | 0.6579 | 0.9336 | 0.7037 | 0.6967 | 0.7348 | 0.6963 |

**r3（标定阈值 0.60）**
| 权重 | segm/mAP | AP50 | AP75 | AR100 | bbox/mAP | comp |
|---|---:|---:|---:|---:|---:|---:|
| bestS(E70) | 0.6693 | 0.9377 | 0.7265 | 0.7094 | 0.6847 | 0.6770 |
| bestB(E250) | 0.6342 | 0.9142 | 0.6647 | 0.6765 | 0.7131 | 0.6736 |
| bestC(E290) | 0.6357 | 0.9163 | 0.6799 | 0.6750 | 0.7123 | 0.6740 |
| last(E300) | 0.6341 | 0.9161 | 0.6781 | 0.6736 | 0.7127 | 0.6734 |

**双口径总清单（2026-09-17 修订版3：12 臂 × 全权重，segm/bbox 各 mAP+AP50+AP75 双口径）**：

| 臂 | 权重 | segm@0.4 mAP/AP50/AP75 | segm@cal mAP/AP50/AP75 | 阈值 | bbox mAP/AP50/AP75 | comp@0.4 | comp@cal |
|---|---|---|---|---:|---|---:|---:|
| P 纯点 | best | 0.6413/0.9264/0.6949 | 0.6430/0.9410/0.6932 | 0.60 | 0.7024/0.9361/0.8142 | 0.6718 | 0.6727 |
| P 纯点 | best_bbox | 0.6245/0.9205/0.6643 | 0.6309/0.9226/0.6800 | 0.60 | 0.7197/0.9298/0.8188 | 0.6721 | 0.6753 |
| P 纯点 | best_composite | 0.6299/0.9256/0.6741 | 0.6315/0.9256/0.6773 | 0.60 | 0.7181/0.9360/0.8189 | 0.6740 | 0.6748 |
| P 纯点 | last | 0.6247/0.9218/0.6576 | 0.6273/0.9218/0.6838 | 0.60 | 0.7161/0.9339/0.8145 | 0.6704 | 0.6717 |
| B 纯框 | best | 0.6486/0.9345/0.6963 | 0.6584/0.9306/0.7068 | 0.55 | 0.7089/0.9406/0.8334 | 0.6788 | 0.6836 |
| B 纯框 | best_bbox | 0.6454/0.9375/0.6816 | 0.6525/0.9412/0.6902 | 0.55 | 0.7214/0.9434/0.8224 | 0.6834 | 0.6869 |
| B 纯框 | best_composite | 0.6454/0.9375/0.6816 | 0.6525/0.9412/0.6902 | 0.55 | 0.7214/0.9434/0.8224 | 0.6834 | 0.6869 |
| B 纯框 | last | 0.6345/0.9217/0.6720 | 0.6435/0.9220/0.6859 | 0.55 | 0.7140/0.9360/0.8118 | 0.6743 | 0.6788 |
| PB 点+框 | best | 0.6530/0.9354/0.7165 | 0.6694/0.9393/0.7273 | 0.60 | 0.7174/0.9352/0.8168 | 0.6852 | 0.6934 |
| PB 点+框 | best_bbox | 0.6411/0.9290/0.6742 | 0.6464/0.9428/0.6834 | 0.60 | 0.7276/0.9416/0.8139 | 0.6843 | 0.6870 |
| PB 点+框 | best_composite | 0.6530/0.9354/0.7165 | 0.6694/0.9393/0.7273 | 0.60 | 0.7174/0.9352/0.8168 | 0.6852 | 0.6934 |
| PB 点+框 | last | 0.6392/0.9233/0.6855 | 0.6407/0.9332/0.6796 | 0.60 | 0.7215/0.9336/0.8255 | 0.6803 | 0.6811 |
| A0 PBM | best | 0.6446/0.9301/0.6918 | 0.6683/0.9358/0.7315 | 0.60 | 0.6963/0.9467/0.8229 | 0.6705 | 0.6823 |
| A0 PBM | best_bbox | 0.6355/0.9362/0.6769 | 0.6451/0.9373/0.6776 | 0.60 | 0.7284/0.9443/0.8105 | 0.6819 | 0.6868 |
| A0 PBM | best_composite | 0.6355/0.9362/0.6769 | 0.6451/0.9373/0.6776 | 0.60 | 0.7284/0.9443/0.8105 | 0.6819 | 0.6868 |
| A0 PBM | last | 0.6331/0.9312/0.6671 | 0.6410/0.9355/0.6693 | 0.60 | 0.7218/0.9370/0.8041 | 0.6775 | 0.6814 |
| A0-sg 隔离 | best | 0.6592/0.9349/0.7152 | 0.6751/0.9438/0.7460 | 0.60 | 0.7183/0.9448/0.8239 | 0.6888 | 0.6967 |
| A0-sg 隔离 | best_bbox | 0.6513/0.9307/0.7029 | 0.6635/0.9366/0.7161 | 0.60 | 0.7376/0.9380/0.8469 | 0.6945 | 0.7006 |
| A0-sg 隔离 | best_composite | 0.6513/0.9307/0.7029 | 0.6635/0.9366/0.7161 | 0.60 | 0.7376/0.9380/0.8469 | 0.6945 | 0.7006 |
| A0-sg 隔离 | last | 0.6483/0.9342/0.6931 | 0.6579/0.9336/0.7037 | 0.60 | 0.7348/0.9399/0.8477 | 0.6915 | 0.6963 |
| R3 渲染 | best | 0.6433/0.9363/0.6780 | 0.6693/0.9377/0.7265 | 0.60 | 0.6847/0.9414/0.8002 | 0.6640 | 0.6770 |
| R3 渲染 | best_bbox | 0.6248/0.9083/0.6554 | 0.6342/0.9142/0.6647 | 0.60 | 0.7131/0.9197/0.7996 | 0.6690 | 0.6736 |
| R3 渲染 | best_composite | 0.6284/0.9096/0.6720 | 0.6357/0.9163/0.6799 | 0.60 | 0.7123/0.9184/0.8036 | 0.6704 | 0.6740 |
| R3 渲染 | last | 0.6271/0.9105/0.6709 | 0.6341/0.9161/0.6781 | 0.60 | 0.7127/0.9181/0.8022 | 0.6699 | 0.6734 |
| A3 v1 | best | 0.6930/0.9444/0.7796 | — | — | 0.6508/0.9333/0.7795 | 0.6719 | — |
| A3 v1 | best_bbox | 0.6686/0.9432/0.7289 | — | — | 0.7240/0.9383/0.8314 | 0.6963 | — |
| A3 v1 | best_composite | 0.6781/0.9540/0.7401 | — | — | 0.7206/0.9394/0.8403 | 0.6993 | — |
| A3 v1 | last | 0.6696/0.9497/0.7295 | — | — | 0.7190/0.9386/0.8280 | 0.6943 | — |
| A3-s45 | best | 0.6808/0.9572/0.7556 | — | — | 0.6936/0.9470/0.8229 | 0.6872 | — |
| A3-s45 | best_bbox | 0.6677/0.9397/0.7259 | — | — | 0.7068/0.9266/0.7959 | 0.6873 | — |
| A3-s45 | best_composite | 0.6677/0.9397/0.7259 | — | — | 0.7068/0.9266/0.7959 | 0.6873 | — |
| A3-s45 | last | 0.6659/0.9361/0.7254 | — | — | 0.7046/0.9238/0.7998 | 0.6853 | — |
| A3sg | best | 0.6981/0.9481/0.7755 | — | — | 0.6879/0.9437/0.7832 | 0.6930 | — |
| A3sg | best_bbox | 0.6605/0.9203/0.7109 | — | — | 0.7135/0.9346/0.8055 | 0.6870 | — |
| A3sg | best_composite | 0.6981/0.9481/0.7755 | — | — | 0.6879/0.9437/0.7832 | 0.6930 | — |
| A3sg | last | 0.6605/0.9279/0.7104 | — | — | 0.7130/0.9369/0.8042 | 0.6868 | — |
| A3sgt | best | 0.6560/0.9388/0.7023 | — | — | 0.7240/0.9476/0.8273 | 0.6900 | — |
| A3sgt | best_bbox | 0.6439/0.9405/0.6883 | — | — | 0.7274/0.9412/0.8304 | 0.6857 | — |
| A3sgt | best_composite | 0.6560/0.9388/0.7023 | — | — | 0.7240/0.9476/0.8273 | 0.6900 | — |
| A3sgt | last | 0.6437/0.9374/0.6842 | — | — | 0.7225/0.9400/0.8294 | 0.6831 | — |
| A3sgtm-dclip | best | 0.6573/0.9190/0.7171 | — | — | 0.7041/0.9221/0.8112 | 0.6807 | — |
| A3sgtm-dclip | best_bbox | 0.6504/0.9280/0.6878 | — | — | 0.7182/0.9346/0.8022 | 0.6843 | — |
| A3sgtm-dclip | best_composite | 0.6565/0.9341/0.7032 | — | — | 0.7156/0.9272/0.7961 | 0.6861 | — |
| A3sgtm-dclip | last | 0.6499/0.9316/0.6961 | — | — | 0.7154/0.9355/0.7929 | 0.6827 | — |
| **A3sgm** | best | 0.6933/0.9540/0.7752 | — | — | 0.6878/0.9502/0.8302 | 0.6905 | — |
| **A3sgm** | best_bbox | 0.6828/0.9453/0.7461 | — | — | 0.7343/0.9396/0.8416 | 0.7085 | — |
| **A3sgm** | best_composite | 0.6828/0.9453/0.7461 | — | — | 0.7343/0.9396/0.8416 | 0.7085 | — |
| **A3sgm** | last | 0.6816/0.9465/0.7492 | — | — | 0.7317/0.9405/0.8438 | 0.7067 | — |

（bbox 三指标与阈值无关，双口径共用；尾臂 segm@cal=—：自标定 0.4。两阶段热启动 dev100：best(E32) segm 0.6650/0.9341/0.7214、bbox 0.7348/0.9417/0.8553、comp 0.6999；last(E33) segm 0.6640/…、comp 0.6994。）
...
| B | `vhr10_p2v2_matrix300_b_box_tr1.0_va1.0` | `inference_{best,best_bbox,best_composite,last}_vhr10`（无臂前缀） | 同上 `worker_b.log` + `["b\|<kind>"]` |
| PB | `vhr10_p2v2_matrix300_pb_tr1.0_va1.0` | `inference_pb_{best,best_bbox,best_composite,last}_model_epoch{155,235,155,300}_vhr10` | `worker_pb.log` + `["pb\|<kind>"]` |
...
每推理目录内：`metrics.json`（完整 COCO）、`predictions.json`（逐检测记录）、`run_manifest.json`（指纹/合同/图像集）。标定口径的 train-split 扫描原始输出在同目录 `calib_<arm>_train.txt`（每点 3 字段：thr/segm-mAP/AP75；a0_sg 为 5 点 0.40–0.60，其余 6 点 0.40–0.65）。探针：`inference/probes/mask_threshold_sweep.py <ckpt> <thrs> [train|validation]`。
