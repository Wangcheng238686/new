...
# mask_downscaling convolution stack to adapt, lifting the single remaining
# frozen bottleneck that stands between the shape-dense signal and the decoder.
#
# Mask downscaling: 3-layer conv stack (4684 params, 0.005M).  Unfreezing it
# lets the PE bridge reinterpret the 98%-sparse coarse-mask canvas for the
# decoder's spatial embedding, instead of processing it through SAM's
# original mask-prompt distribution (which was trained on strong, full-image
# masks).
#
...
# Unfreeze the frozen mask_downscaling conv layers.
export UNFREEZE_MASK_DOWNSCALING=1
