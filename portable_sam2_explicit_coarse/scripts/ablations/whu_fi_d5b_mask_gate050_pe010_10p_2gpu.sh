#!/usr/bin/env bash
# D5-B: dense PE-read adaptation on top of D5-A
# (per docs/d5_review_handoff.md §3-§4).  Differences vs D5-A (whu_fi_d5a_*):
#   PROMPT_ENCODER_TRAIN_MASK_DOWNSCALING=1  — unfreeze ONLY the PromptEncoder
#     mask-input convolution stack (4,684 params; point/box embeddings stay
#     frozen, asserted by the ablation contract validator);
#   PROMPT_ENCODER_LR_MULT=0.1  — PE group base LR 5e-4*0.1=5e-5; a nonzero
#     multiplier is REQUIRED, unfreeze alone never updates the weights.
# Verification hooks: the prompt-pathway monitor reports a dedicated
# pe_mask_downscaling branch (final-mask grad RMS + epoch update RMS), and
# the optimizer audit logs the prompt_encoder group with its LR.
set -Eeuo pipefail
...
export RUN_TAG="whu_d5b_mask_gate050_pe010_10p_2gpu"
