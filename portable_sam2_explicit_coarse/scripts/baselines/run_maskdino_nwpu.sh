#!/bin/bash
# =============================================================================
# MaskDINO R50 @ NWPU VHR-10 统一启动脚本 (官方 IDEA-Research 实现, mdino 环境)
#   mdino = /data2/wangcheng/envs/mdino (torch1.13.1+cu117 + detectron2 v0.6
#           + MSDeformAttn 已编译)
#
# 用法: bash run_maskdino_nwpu.sh [mode] [gpu] [ckpt(test 模式用)]
#   mode: smoke(默认) | train | resume | test
#   gpu : 默认 3
#
# 协议: train 520 全量不滤空图 / val 130 每10ep 评(日志选best), val 兼任 test 终评
#       / COCOEvaluator maxDets=[1,10,100] / 不开 TTA
# 超参: WHU-1024 版同款官方 COCO 配方 bs16 AdamW 1e-4 LSJ-1024 AMP grad-clip;
#       单卡 bs2 x accum8 = 有效16; 300 NWPU ep, 里程碑按 COCO 原比例
# =============================================================================
set -uo pipefail
...
CFG="configs/nwpu/maskdino_R50_nwpu.yaml"
RUNTAG="maskdino_R50_nwpu_bs2ai8_300ep"
...
case "${MODE}" in
  smoke)
    ...
    start_gpu_monitor
    timeout 600 "${MDINO_PY}" train_nwpu.py --config-file "${CFG}" --accum 8 \
  ...
  train)
    start_gpu_monitor
    "${MDINO_PY}" train_nwpu.py --config-file "${CFG}" --accum 8 2>&1 | tee -a "${LOG}"
    echo "[train] exit=${PIPESTATUS[0]}" | tee -a "${LOG}"
    ;;
  resume)
    start_gpu_monitor
    "${MDINO_PY}" train_nwpu.py --config-file "${CFG}" --accum 8 --resume 2>&1 | tee -a "${LOG}"
    echo "[resume] exit=${PIPESTATUS[0]}" | tee -a "${LOG}"
    ;;
  test)
    [ -z "${CKPT}" ] && { echo "[test] 用法: run_maskdino_nwpu.sh test <gpu> <ckpt.pth>" | tee -a "${LOG}"; exit 1; }
    echo "[test] ckpt: ${CKPT}  (val 兼任 test 终评)" | tee -a "${LOG}"
    "${MDINO_PY}" "${PROJ_ROOT}/scripts/baselines/eval_maskdino_nwpu.py" "${CKPT}" \
      --config "${CFG}" --out-dir "$(dirname "${CKPT}")/test_out" 2>&1 | tee -a "${LOG}"
