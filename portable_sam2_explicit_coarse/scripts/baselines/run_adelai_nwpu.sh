#!/bin/bash
# =============================================================================
# AdelaiDet 基线 @ NWPU VHR-10 统一启动脚本 (环境: dt2)
# 覆盖: condinst | solov2   (WHU-1024 版同款 1x 配方, bs8 lr0.005, ~300ep 折算)
#
# 用法: bash run_adelai_nwpu.sh <condinst|solov2> [mode] [gpu]
#   mode: smoke(默认) | train | resume | test
#   gpu : 默认 1
# 协议: train 520 全量(3178 实例) / val 130(743 实例) 选模, val 兼任 test 终评
# =============================================================================
set -uo pipefail
...
case "${MODEL}" in
  condinst)
    CFG="configs/NWPU/condinst_MS_R_50_1x_nwpu.yaml"
    RUNTAG="condinst_MS_R50_1x_nwpu_bs8lr05m";;
  solov2)
    CFG="configs/NWPU/solov2_R50_1x_nwpu.yaml"
    RUNTAG="solov2_R50_1x_nwpu_bs8lr05m";;
...
[ -z "${WORKDIR}" ] && WORKDIR="/data1/wangcheng/checkpoint/nwpu_vhr10_baselines/${MODEL}"
...

{
  ...
  echo "repo: ${REPO}  head: $(git log --oneline -1 2>/dev/null) (+local patch: adet/data/nwpu_builtin.py)"
