#!/bin/bash
# =============================================================================
# RSPrompter (query) @ NWPU VHR-10 对比实验（PromptMiner-SAM2 协议对齐版）
#
# 用法: bash run_rsprompter_nwpu.sh [mode] [gpu]
#   mode: smoke | train(300ep, AMP fp16, bs1x累积2) | resume | test
#   gpu : 默认 1
# 协议: train 520 全量(3178 实例) / val 130 选 best(segm_mAP), val 兼任 test
#       终评 / maxDets 100 / 10 类
# 超参: WHU-1024 版同款论文配方 AdamW 2e-4 wd0.05 + clip0.1, LSJ-1024+hflip0.5,
#       300ep cosine
...
CFG="configs/rsprompter/rsprompter_anchor-nwpu-1024.py"
WORKDIR="/data1/wangcheng/checkpoint/nwpu_vhr10_baselines/rsprompter/rsprompter_anchor_nwpu_bs1ai2_300ep"
RUNTAG="rsprompter_anchor_nwpu_bs1ai2_300ep"
...

{
  ...
  echo "data_root: /data1/wangcheng/dataset/NWPU VHR-10 dataset (train 520 full / val 130, val=test)"
...
case "${MODE}" in
  ...
  resume)
    ...
    ;;
  test)
    # val 兼任 test 终评; ckpt 缺省取 work_dir 下 best_*.pth (segm_mAP)
    if [ -z "${CKPT}" ]; then
      CKPT=$(ls -t "${WORKDIR}"/best_*.pth "${WORKDIR}"/epoch_*.pth 2>/dev/null | head -1)
    fi
    [ -z "${CKPT}" ] && { echo "[test] 未找到 ckpt(用法: run_rsprompter_nwpu.sh test <gpu> <ckpt.pth>)" | tee -a "${LOG}"; exit 1; }
    echo "[test] ckpt: ${CKPT}" | tee -a "${LOG}"
    python tools/test.py "${CFG}" "${CKPT}" \
      --work-dir "${WORKDIR}/test_out" 2>&1 | tee -a "${LOG}"
