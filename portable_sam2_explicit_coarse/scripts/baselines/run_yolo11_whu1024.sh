#!/bin/bash
# =============================================================================
# YOLO11-seg (ultralytics 8.4, YOLO11s-seg) @ WHU-1024 统一启动脚本
# 环境: cvt2 (ultralytics 以 --no-deps 安装, 不动 mm 栈)
#
# 用法: bash run_yolo11_whu1024.sh [mode] [gpu]
#   mode: convert(仅转标签) | smoke(默认) | train | resume | test
#   gpu : 默认 0
#
# 协议: train 2943 全量(339 空背景图保留为负样本) / val 627 选 best /
#       test 2220 终评 maxDets=100 / 不开 TTA
...
DATA_YAML="/data1/wangcheng/dataset/WHU/yolo_seg/data.yaml"
RUNDIR="/data1/wangcheng/checkpoint/whu1024_baselines/yolo11seg/yolo11s_seg_whu1024_default"
LOGDIR="${PROJ_ROOT}/logs/baselines"
mkdir -p "${LOGDIR}" "${RUNDIR}"

RUNTAG="yolo11s_seg_whu1024_default"
...
case "${MODE}" in
  convert)
    python "${SCRIPTS}/coco2yolo_seg.py" 2>&1 | tee -a "${LOG}"
    ;;
  smoke)
...
m = YOLO('/home/wangcheng/project/new/yolo11s-seg.pt')
...
m.train(data='${DATA_YAML}', device=0, project='${RUNDIR}',
        name='yolo11s_seg_whu1024_default', seed=0, exist_ok=True, verbose=True)
EOF
  ...
  resume)
    ...
    python - <<EOF 2>&1 | tee -a "${LOG}"
from ultralytics import YOLO
m = YOLO('${RUNDIR}/yolo11s_seg_whu1024_default/weights/last.pt')
m.train(resume=True, device=0)
EOF
    echo "[resume] exit=$?" | tee -a "${LOG}"
    ;;
  test)
    BEST="${RUNDIR}/yolo11s_seg_whu1024_default/weights/best.pt"
    [ -f "${BEST}" ] || { echo "[test] 未找到 best.pt" | tee -a "${LOG}"; exit 1; }
    echo "[test] ckpt: ${BEST}" | tee -a "${LOG}"
    python "${SCRIPTS}/eval_yolo_whu.py" "${BEST}" \
      --out-dir "${RUNDIR}/yolo11s_seg_whu1024_default/test_out" 2>&1 | tee -a "${LOG}"
