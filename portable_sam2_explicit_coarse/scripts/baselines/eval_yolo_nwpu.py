#!/usr/bin/env python
"""NWPU VHR-10 val 推理与评估 (YOLO11-seg, ultralytics)

用 best.pt 在 val 130 (NWPU 无独立 test, val 兼任终评) 上预测 -> 输出 COCO json
-> pycocotools 评估 bbox 与 segm 的 mAP / AP50 / AP75 / APs / APm / APl,
maxDets=100, 10 类 (COCO category_id 1-10)。

用法:
  python eval_yolo_nwpu.py CKPT [--out-dir DIR] [--vis] [--vis-limit 20]
"""
import argparse
import json
import os
...
GT_JSON = '/data1/wangcheng/dataset/NWPU VHR-10 dataset/coco_split/NWPU_instances_val.json'
IMG_DIR = Path('/data1/wangcheng/dataset/NWPU VHR-10 dataset/yolo_seg/images/val')
...
def predict_to_coco(model, coco_gt, max_images=0):
    ...
    for r in results:
        ...
        clsses = r.boxes.cls.cpu().numpy().astype(int) if r.boxes is not None else np.zeros(0, int)
        ...
        for i in range(len(boxes)):
            ...
            if seg is None:
                continue
            preds.append(dict(
                image_id=image_id, category_id=int(clsses[i]) + 1,  # YOLO cls 0-9 -> COCO 1-10
