...
export DENSE_CAPACITY_HEAD_ENABLED="${DENSE_CAPACITY_HEAD_ENABLED:-0}"
export DENSE_CAPACITY_MID_CHANNELS="${DENSE_CAPACITY_MID_CHANNELS:-256}"
export DENSE_CAPACITY_AUX_WEIGHT="${DENSE_CAPACITY_AUX_WEIGHT:-0.10}"
...
DCLIP_ARGS=()
if [[ "${DECOUPLED_AUX_GRAD_CLIP:-0}" = "1" ]]; then
  DCLIP_ARGS=(--decoupled-aux-clip)
fi
TAIL_ARGS=()
...
DENSE_CAPACITY_ARGS=()
if [[ "${DENSE_CAPACITY_TRAIN_ONLY:-0}" = "1" ]]; then
  if [[ "${DENSE_CAPACITY_HEAD_ENABLED:-0}" != "1" ]]; then
    echo "DENSE_CAPACITY_TRAIN_ONLY=1 requires DENSE_CAPACITY_HEAD_ENABLED=1" >&2
    exit 2
  fi
  DENSE_CAPACITY_ARGS=(--train-dense-capacity-only)
fi
...
echo "  dense_capacity: enabled=${DENSE_CAPACITY_HEAD_ENABLED} mid=${DENSE_CAPACITY_MID_CHANNELS} aux=${DENSE_CAPACITY_AUX_WEIGHT} train_only=${DENSE_CAPACITY_TRAIN_ONLY:-0}"
