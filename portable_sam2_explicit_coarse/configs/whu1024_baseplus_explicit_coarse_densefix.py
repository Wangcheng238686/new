        ...
        mask_head=dict(
            ...
            **_roi_sam_override,
            ...
            shape_prior_cfg=dict(
                ...
                # densefix: bypass the learnable global-sigmoid gate (which
                # stayed pinned near its 0.25 init across 8 epochs, leaving the
                # dense prompt at a ~0.13 applied_delta_ratio) and instead use a
                # fixed, untrained interpolation coefficient of 0.5 (2x the
                # stuck value). This isolates "the gate never opened" as the
                # sole changed variable against C4.
                shape_scale_mode="fixed",
                prompt_scale_init=float(
                    os.environ.get("SHAPE_DENSE_ALPHA_INIT", "0.5")
                ),
            ),
